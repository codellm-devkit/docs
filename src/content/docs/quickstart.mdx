...
1. **Install CLDK.**
   ...
   The Java backend ships with the package. To analyze Java projects, you need only a JDK on your `PATH`.
...
2. **Get a sample project.**
   ...
   <Aside type="note">
   Any Java project on disk works here. `JAVA_APP_PATH` must point at the root of the project. Commons CLI is small and well structured, so it analyzes quickly.
   </Aside>

3. **Run your first analysis.**

   Build a CLDK object for Java. Point it at the project. Print the class count and the call graph. Call graphs, callers, and callees need `analysis_level=AnalysisLevel.call_graph`. The default level builds only the symbol table.
...
One call converts a directory of source files into a typed `analysis` object. Then `get_method(cls, sig)` returns the source body of a method. `get_callers` and `get_callees` walk the graph. The [concepts guide](/guides/concepts/) covers the object model. The [cheat sheet](/resources/cheatsheet/) lists the available methods.
