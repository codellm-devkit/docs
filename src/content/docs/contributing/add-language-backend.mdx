...
This guide describes how to add first-class Go support to CLDK by building the `codeanalyzer-go` backend and wiring it into the Python SDK. It covers the tooling, schema, and architecture decisions involved.

<Aside type="tip" title="Claude Code plugin in progress">
CLDK ships a Claude Code plugin that scaffolds first-class support for a new language end to end: a `codeanalyzer-<lang>` backend plus SDK bindings, validated through a level-1 analysis branch. See [codellm-devkit/developer-skillset](https://github.com/codellm-devkit/developer-skillset). This page is the manual walkthrough behind what the plugin automates.
</Aside>

## Architecture overview

CLDK's two-layer design separates the backend analyzer (emits `analysis.json`) from the SDK frontend (a Python query API):
...
Before writing code, choose the runtime, parser, resolver, materialization format, and packaging. Go provides a standard tool for each of these.

<Aside type="tip">
**Recommended tooling for Go:**
- Runtime: Go
- Structural parser: `go/ast` (the stdlib AST, always available with the Go toolchain)
- Resolver (Tier 1): `go/packages` + `go/types` (the standard library's source-level resolver: precise and performant)
...
Design the analyzer's data types by mirroring the canonical schema and adding Go-specific nodes. Go's structure differs from Java and Python: it has no classes but uses structs and interfaces, methods always carry a receiver type, interfaces are structural, and packages are the module unit.
...
Resolve each unresolved call site using `go/types`. This is **level 1** and inexpensive because the type resolver is already loaded.
...
Add `cldk/analysis/go/` with the `GoAnalysis` facade and backend wrapper. The facade takes a `backend=` **config object** and dispatches on its **type** to choose a concrete backend — exactly how `JavaAnalysis`, `PythonAnalysis`, and `TypeScriptAnalysis` work. There is no `analysis_backend_path` (the binary ships packaged) and no `analysis_json_path` (caching is folded into the config's `cache_dir`):
...
Dispatch no longer lives in a single `if self.language == ...` ladder inside `CLDK.analysis()`. CLDK now selects behavior along **two orthogonal axes**: the *language* is chosen by **which factory method you call** (`CLDK.go(...)`), and the *backend* is chosen by the **type** of the `backend=` config object you pass. Registering a language therefore means three small additions, not one branch.

<Aside type="caution" title="The old pattern is gone">
Earlier versions told you to add `elif self.language == "go": ...` inside `CLDK.analysis()`. That `if/elif`-on-language dispatch no longer exists. `CLDK(language).analysis(...)` is now a deprecated compatibility **shim** that forwards to the factory methods. Add a factory method instead.
</Aside>

#### Step 11a: Add the backend-config type(s)

In `cldk/analysis/commons/backend_config.py`, add a language-specific codeanalyzer config (subclassing `CodeAnalyzerConfig` so it inherits `cache_dir`) plus a per-language `GoBackend` union the facade matches on. If Go will never have a Neo4j backend, the union is just the codeanalyzer config; if it might, union in `Neo4jConnectionConfig` (already defined there):
...
In `cldk/core.py`, add a `@staticmethod` factory alongside `CLDK.java`, `CLDK.python`, and `CLDK.typescript`. It normalizes the project path and constructs the `GoAnalysis` facade, forwarding the `backend=` config untouched (the facade picks the concrete backend from its type):
...
`CLDK(language).analysis(...)` is retained only for backward compatibility and is deprecated. Add a branch that forwards to your new factory, mapping the legacy keyword args onto a `backend=` config (folding the removed `analysis_json_path` into `cache_dir`, and ignoring the removed `analysis_backend_path`):
...
New code should call `CLDK.go(...)` directly; the shim exists so existing `CLDK(language="go").analysis(...)` callers keep working.
