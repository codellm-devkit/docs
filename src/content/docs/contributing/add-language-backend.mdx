...
description: "Step-by-step guide to add first-class Go support to CLDK by hand: the backend analyzer and the Python SDK frontend."
---
...
CLDK ships a Claude Code plugin that scaffolds first-class support for a new language: a `codeanalyzer-<lang>` backend and the SDK bindings, validated through a level-1 analysis branch. See [codellm-devkit/developer-skillset](https://github.com/codellm-devkit/developer-skillset). This page is the manual walkthrough of the same work.
</Aside>

## Architecture overview

The two-layer design of CLDK separates the backend analyzer, which emits `analysis.json`, from the SDK frontend, which is a Python query API:
...
This is **separate-tool architecture**. `go/ast` does the parsing. `go/types` does the type resolution. Both are in the standard library, so the analyzer has few dependencies.
</Aside>

Record these choices in the `README.md` of your analyzer, under an **Architecture and Tooling** heading, so that they stay recoverable:
...
If it is not present, stop. Tell the user to install it from [golang.org/dl](https://golang.org/dl).

### Step 2: Define the Go schema

Design the data types of the analyzer. Mirror the canonical schema, then add Go-specific nodes. Go differs from Java and Python in four ways. It has no classes, but it has structs and interfaces. Methods always carry a receiver type. Interfaces are structural. Packages are the module unit.
...
Walk the Go packages of the project and build `symbol_table: Dict[file_path, GoModule]`. Follow the pattern in the Python analyzer.
...
3. Per file:
   ...
   - Check the cache by file hash or mtime. If the file is unchanged, reuse the cached `GoModule` and go to the next file.
...
**Signature canonicalization** (the primary identity mechanism):
...
- The output validates against the SDK `GoApplication` Pydantic model. You create that model in Part 2.
...
### Step 6: Call graph construction (resolver-based)

Resolve each unresolved call site with `go/types`. This step is **level 1**, and it is cheap because the type resolver is already loaded.
...
Give the analyzer a command-line interface that matches what the SDK expects:
...
Version it and distribute it, either bundled in the SDK or downloaded on first use. Add a `VERSION` file, or a tag in Git:
...
Add `cldk/analysis/go/` with the `GoAnalysis` facade and the backend wrapper. The facade takes a `backend=` **config object** and dispatches on its **type** to choose a concrete backend. `JavaAnalysis`, `PythonAnalysis`, and `TypeScriptAnalysis` all work the same way. There is no `analysis_backend_path`, because the binary ships packaged. There is no `analysis_json_path`, because `cache_dir` on the config holds the cache:
...
Import `cache_subdir` from the shared backend-config module:
...
CLDK selects behavior along **two independent axes**. The factory method that you call selects the *language*, for example `CLDK.go(...)`. The **type** of the `backend=` config object that you pass selects the *backend*. To register a language, you therefore make three small additions, not one branch.

<Aside type="caution" title="Do not add a language branch">
Do not add `elif self.language == "go": ...` inside `CLDK.analysis()`. There is no `if/elif` dispatch on language. `CLDK(language).analysis(...)` is a deprecated compatibility **shim** that forwards to the factory methods. Add a factory method instead.
</Aside>

#### Step 11a: Add the backend-config types

In `cldk/analysis/commons/backend_config.py`, add a language-specific codeanalyzer config. It subclasses `CodeAnalyzerConfig`, so it inherits `cache_dir`. Then add a per-language `GoBackend` union that the facade matches on. If Go will never have a Neo4j backend, the union holds only the codeanalyzer config. If Go can have one, add `Neo4jConnectionConfig`, which is already defined there:
...
In `cldk/core.py`, add a `@staticmethod` factory beside `CLDK.java`, `CLDK.python`, and `CLDK.typescript`. It normalizes the project path and builds the `GoAnalysis` facade. It forwards the `backend=` config without a change, and the facade picks the concrete backend from its type:
...
`CLDK(language).analysis(...)` exists only for backward compatibility, and it is deprecated. Add a branch that forwards to your new factory. Map the legacy keyword arguments onto a `backend=` config. Put the removed `analysis_json_path` into `cache_dir`, and ignore the removed `analysis_backend_path`:
...
New code must call `CLDK.go(...)` directly. The shim exists so that existing `CLDK(language="go").analysis(...)` callers keep working.
...
If you distribute it as a bundled binary, add it to `cldk/analysis/go/codeanalyzer/bin/`. You can also arrange a download on first use.
...
7. **Modularity checklist:**
   ...
   - [ ] Core orchestrator delegates to phases. It never inlines.
...
8. **Schema integrity:**
   ...
   - [ ] Receiver types are captured, so that `*T.Method` differs from `T.Method`.
...
- **Level-2 (CodeQL) backend**: The stub is ready. Implement the CodeQL Go pack integration for deeper dataflow analysis.
