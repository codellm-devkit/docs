...
A single-page reference for the common CLDK calls. Construct one `analysis` object, then query it. For background and walkthroughs, see [What is CLDK?](/what-is-cldk/), [Core concepts](/guides/concepts/), and the [Quickstart](/quickstart/).
...
The same pattern applies to every language: `CLDK(language=...).analysis(...)`. The analysis level determines how much is computed.
...
| --- | --- | --- |
| `language` | `"java"` / `"python"` | Selects the analysis backend. |
