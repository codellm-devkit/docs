...
description: Install and initialize the cldk Python package to start analyzing your codebases.
---
...
<Tabs>
  ...
  <TabItem label="TypeScript Analysis">
    For TypeScript and JavaScript analysis, CLDK relies on a companion backend, [`codeanalyzer-ts`](/backends/codeanalyzer-ts/). It ships as a prebuilt, self-contained binary and does **not** require Bun or Node to run. Install the `codeanalyzer-typescript` package, which adds a `cants` command to your `PATH`.
