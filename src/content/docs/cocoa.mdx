...
**COCOA** (**CO**de **CO**ntext **A**gent) is a [Claude Code plugin](https://code.claude.com/docs/en/plugins). It runs CLDK before it answers a question about code structure. It uses Bash heredocs to ask:

- which methods call a given method
- which methods that method calls
- whether a sink is reachable from a source
- which call sites a change can affect

This guide builds COCOA by hand: a manifest, a code-context subagent, and two skills. All three use the same CLDK heredoc. The technical id of the plugin is the lowercase `cocoa`, so the skills are `/cocoa:callers` and `/cocoa:reachable`.

<Aside type="note" title="Prerequisites">
You need [Claude Code](https://code.claude.com/docs/en/quickstart) and the `cldk` package in the Python that `python3` on your `PATH` runs. On CLDK 2.0, install the Java backend with `pip install --pre 'cldk[java]'`. On CLDK 1.5.0, `pip install cldk` includes the Java backend. Both versions provide their own JVM, so you do not need a separate JDK. If the `analysis` object is new to you, read [What is CLDK?](/what-is-cldk/) first.
</Aside>

## How COCOA works

COCOA uses one mechanism. The Bash tool of Claude Code can run Python, so COCOA runs a one-shot CLDK script and reads the stdout. COCOA needs no SDK integration and no server.
...
The plugin adds two components on top of this mechanism:

- a **subagent** (`code-context`) for open questions about the code
- **skills** (`/cocoa:callers`, `/cocoa:reachable`), which are named operations that you can run directly

### The mechanism: CLDK in a heredoc
...
Call-graph queries need `analysis_level=AnalysisLevel.call_graph`. Each heredoc runs in a new process, so CLDK constructs the `analysis` object again on every call. CLDK caches the backend output on disk (`<project>/.codeanalyzer`), so only the first call runs the backend.
</Aside>

## COCOA in action

After you install COCOA, you stay in Claude Code. Claude can delegate to the `code-context` agent, or you can run a skill such as `/cocoa:reachable` directly. Three cases follow.

### Generate a unit test from the callees of a method

COCOA supplies the source body of the target (`get_method`) and the methods that it calls (`get_callees`). The generated test then uses the real dependencies and signatures of the method.
...
The test comes from the resolved callees, not from an assumed API.

### Assess change impact before you edit

Before a signature change, COCOA lists the callers of the method (`get_callers`). You can then limit the edit to the call sites that use it.
...
To understand a new codebase, walk the call graph (`get_callers` and `get_callees`) instead of single files.
...
COCOA builds the trace from the call-graph edges.

> The pattern is the same in all three cases. A prompt starts a CLDK query, and Claude answers from the methods or paths that come back. The rest of this page builds that plugin.

## Build the plugin

<Steps>

1. **Scaffold the layout.** A plugin is a directory. Only `plugin.json` goes inside `.claude-plugin/`. Everything else sits at the root.
...
3. **Add the Code Context subagent**: `agents/code-context.md`. The frontmatter declares the name of the subagent, when Claude must delegate to it, and which tools it can use. `Bash` is the important one, because it runs the heredoc. The body documents the CLDK pattern that the subagent follows.
...
4. **Add skills for named operations.** The plugin namespaces its skills, so these become `/cocoa:callers` and `/cocoa:reachable`. `$ARGUMENTS` is the text that the user types after the command.
...
5. **(Optional) Greet on session start** with a hook: `hooks/hooks.json`. The hook reminds users that the commands are available.
...
6. **Run it locally.** Point Claude Code at the directory. You do not need to install it during development:

   ```bash
   claude --plugin-dir ./cocoa
   ```

   Then delegate to the agent or run a skill:
   ...
   After you edit any file in the plugin, run `/reload-plugins` to load the change.

7. **Publish it.** Validate the plugin. Then add a marketplace entry so that others can install it with the `/plugin` command.
...
The subagent is the default path for open questions about code. The skills are named commands (`/cocoa:callers`) for specific operations. Both run the same CLDK heredoc.
</Aside>

## Where to go next

<CardGrid>
  <LinkCard title="Common tasks" description="The individual CLDK calls inside the COCOA heredocs." href="/guides/common-tasks/" />
  <LinkCard title="Core concepts" description="Call graphs and analysis levels: the facts that COCOA returns." href="/guides/concepts/" />
