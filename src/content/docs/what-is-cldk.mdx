...
1. **Construct a `CLDK` object** for the target language, for example `CLDK(language="java")`. This selects the analysis backend; no analysis is performed yet.

2. **Build the `analysis` object** by pointing it at a project: `.analysis(project_path="commons-cli")`. The backend runs at this step and produces the program model.
