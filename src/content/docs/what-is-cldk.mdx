...
You query every language through the same `analysis` API. A different backend answers for each one. CLDK supports Java, Python, and TypeScript, and all three are production-ready. Java goes deepest, because it adds class-hierarchy analysis. TypeScript also analyzes JavaScript sources, and it detects framework entrypoints.

| Language   | Status         | Symbol table | Call graph / callers / callees |
| ---------- | -------------- | ------------ | ------------------------------ |
| Java       | Supported      | Yes          | Yes                            |
| Python     | Supported      | Yes          | Yes                            |
| TypeScript | Supported      | Yes          | Yes                            |
| JavaScript | Supported      | Yes          | Yes                            |

Infrastructure-as-code (Helm) has a beta backend, [codeanalyzer-iac](/backends/codeanalyzer-iac/). It has no `CLDK.iac(...)` factory yet, so you run `caniac` as a command. See [The codeanalyzer backends](/backends/) for every backend and its maturity.
