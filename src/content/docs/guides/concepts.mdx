...
CLDK analyzes a project and gives you structured program facts through one `analysis` object. These facts answer questions about structure, such as which methods call a given method. You do not read the source files yourself.

This page describes the two kinds of facts that CLDK produces: **symbol tables** and **call graphs**. It also describes the **analysis levels** that control how much work CLDK does. Every Java example uses [Apache Commons CLI](https://commons.apache.org/proper/commons-cli/) (`project_path="commons-cli"`), the same checkout as [Quickstart](/quickstart/) and [COCOA](/cocoa/). Python examples use a generic `my_pkg`.
...
Use it to list or find code structure: the classes in a project, the source body of a method, or the fields and signatures of a class.
...
CLDK 1.x also has a C facade, `CLDK.c(project_path)`. <Badge text="1.x only" variant="note" /> It gives you the symbol table through `get_c_application()` and `get_functions()` (function name -> `CFunction`), but it has **no** call graph. CLDK 2.0 removes the C facade.
...
The **analysis level** controls how much work CLDK does when it builds the `analysis` object. There are four. Each level is a superset of the level below it.

- `AnalysisLevel.symbol_table`: the **default**. It resolves structure (classes, methods, fields, and signatures). It is fast, and it is enough for symbol-table queries.
- `AnalysisLevel.call_graph`: it also computes call edges. This level is **required** for `get_call_graph`, `get_callers`, and `get_callees`.
- `AnalysisLevel.program_dependency_graph`: it also builds dataflow inside each callable. <Badge text="New in 2.0" variant="caution" />
- `AnalysisLevel.system_dependency_graph`: it also builds dataflow across call boundaries. <Badge text="New in 2.0" variant="caution" />

Levels 1 and 2 are the two levels that CLDK 1.x has. Levels 3 and 4 arrive in CLDK 2.0, which is a release candidate today. See [Dataflow graphs](/guides/dataflow/) for what they build and [Taint analysis](/guides/taint/) for the queries over them.
...
CLDK 1.x has a single-file Java mode, `CLDK.java(source_code="...")`, for quick syntactic work. CLDK 2.0 removes this mode, so use `project_path`. Python needs `project_path` too, unless you pass a `Neo4jConnectionConfig` backend. Java, Python, and TypeScript (`CLDK.typescript(...)`) give you the same symbol-table and call-graph methods through `analysis`.
...
<CardGrid>
  ...
  <LinkCard title="Dataflow graphs" description="Levels 3 and 4: control flow, data dependence, slicing, and flow paths." href="/guides/dataflow/" />
