...
The Python analysis backend provides `PythonAnalysis` in CLDK. It runs **Jedi** for semantic analysis, optional **CodeQL** for call-graph augmentation, and **Tree-sitter** for syntax parsing. The output is a canonical `PyApplication` schema. The schema ships with the backend, and the CLDK Python SDK re-exports it.
...
- **Call graph**: Inter- and intra-procedural call edges (`PyCallEdge`). Jedi gives the baseline. Optional CodeQL augmentation merges resolved callees from both engines.
- **Class hierarchies**: Base classes, inheritance chains, and method overrides.
- **Entrypoints**: Framework-detected entry points such as Flask routes, Celery tasks, Django views, and gRPC servicers, each linked to its callable.
...
- **`codeanalyzer.semantic_analysis.call_graph`**: Builds inter-procedural call graphs with the Jedi resolution. It merges CodeQL edges when you enable them.
...
<Aside type="note">
The CLI is for backend developers and direct analysis runs. CLDK SDK users reach the backend through `CLDK.python(...)`, which manages the virtualenv and runs the analysis internally.
</Aside>
...
```
Merges Jedi edges with CodeQL-resolved edges. CodeQL integration is experimental and can take longer.
...
```
Rebuilds the virtualenv and the analysis cache from the start, in `/tmp/analysis-cache/.codeanalyzer`.
...
2. **CLI invocation**: The SDK builds a `canpy` command with options such as `--codeql`, `--eager`, and `--cache-dir`, then runs it as a subprocess. It parses stdout as `analysis.json`.
...
The **type** of the `backend=` config that you pass to `CLDK.python(...)` selects the backend:
...
`CLDK.python(...)` accepts the `project_path`, `analysis_level`, `target_files`, and `eager` keyword arguments. The `CLDK(language="python").analysis(...)` form still works, but it is **deprecated**. Use `CLDK.python(...)`. The `from cldk import CLDK` import does not change.
...
- **Virtualenv**: Created automatically under the cache directory. The backend installs dependencies from `requirements.txt`, `pyproject.toml`, `setup.py`, and `Pipfile`.
- **Analysis cache**: Indexed by file content hash. Unchanged files reuse cached results.
- **CodeQL database**: If you enable CodeQL, the database is stored under the cache directory and downloaded on first use.

To force a clean rebuild, pass `eager=True` to `CLDK.python(...)`, or `--eager` on the CLI. You can also remove the cache directory.

## Design principles

- **One schema**: The CLI, the SDK, and the consuming code all use the same `PyApplication` schema. All Py* types are Pydantic models with JSON and msgpack serialization.
...
- **Optional CodeQL**: Jedi alone resolves about 80-90% of call edges. CodeQL adds dynamic and RPC calls, but it costs more analysis time. Enable it when you need those edges.
