...
An experimental, opt-in pass uses [**Jelly**](https://github.com/cs-au-dk/jelly) (a flow-based call-graph analyzer for JavaScript/TypeScript) to resolve the higher-order, callback, and dynamic-dispatch edges the type checker misses. Select it with `--call-graph-provider jelly` (or `both` to diff against the `tsc` resolver); `tsc` remains the default. Jelly ships inside the `cants` binary, so no extra install is needed.
</Aside>
...
The read-only backend queries a graph that codeanalyzer-ts itself populates with `cants --emit neo4j`. Running analysis as scheduled jobs and serving agents from the shared graph is the [analysis-at-scale](/at-scale/) workflow.
</Aside>
...
- **Neo4j graph output**: Available; `--emit neo4j` / `--emit schema` (see [Analysis at scale](/at-scale/))
