...
codeanalyzer-ts is **production-ready**, at the same tier as the Java and Python backends. The symbol table and the call graph are stable, and you query them through the Python SDK. Entrypoint detection and the level 3 and 4 dataflow graphs are available.
