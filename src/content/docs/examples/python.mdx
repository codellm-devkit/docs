...
Set `use_codeql=False` to skip CodeQL augmentation and analyze faster (Jedi-only call graph). Defaults to `True` for enhanced inter-procedural accuracy. Also set `cache_dir` to control where the backend stores its virtualenv and analysis cache.
</Aside>
