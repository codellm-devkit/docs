...
<Aside type="caution" title="Not available in CLDK 2.0">
codeanalyzer-java 3.0.1 and newer (schema v2) do not emit CRUD operations. On CLDK 2.0, `get_all_crud_operations` and the `get_all_*_operations` variants raise `CodeanalyzerExecutionException`. The code in this section works on CLDK 1.5.0 only.
</Aside>

Find the methods that read from or write to persistent storage: databases, files, or external services.
...
for crud_summary in crud_operations:
    (class_name, jtype), (method_name, jcallable) = [
        (k, v) for k, v in crud_summary.items() if k != "crud_operations"
    ]
...
<CardGrid>
  ...
  <LinkCard title="Common tasks" description="Task-oriented snippets: list methods, get a method body, build a call graph, find callers, map the class hierarchy." href="/guides/common-tasks/" />
