...
These examples come from the CLDK SDK test suite. Each snippet runs against a real codebase, the recurring sample [Apache Commons CLI](https://commons.apache.org/proper/commons-cli/) (`project_path="commons-cli"`). Each snippet shows one feature and the output that the SDK returns. To adapt an example, use your own project path.
...
Get the list of parameters for a method, with their types and their location in the source.
...
Get every class that extends a target class, including transitive subclasses.
...
Find the public methods that are entry points into the application, for example servlet handlers, REST endpoints, and main methods.
